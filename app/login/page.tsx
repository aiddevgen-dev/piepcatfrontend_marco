...
function LoginForm() {
  ...
  justify-center text-white font-bold instrument italic text-xl">
            ...
            {/* Messages */}
            {verified && (
                <p className="text-green-500 text-sm mb-4">Email verified successfully! You can now sign in.</p>
            )}
            {message === "check-email" && (
                <p className="text-blue-500 text-sm mb-4">Please check your email to verify your account.</p>
            )}
            {errorParam === "invalid-token" && (
                <p className="text-red-500 text-sm mb-4">Invalid or expired verification link.</p>
            )}
            {errorParam === "missing-token" && (
                <p className="text-red-500 text-sm mb-4">Verification token missing.</p>
            )}
            {resendSuccess && (
                <p className="text-green-500 text-sm mb-4">Verification email sent! Please check your inbox.</p>
            )}

            {/* Google OAuth */}
            <Button variant="outline" className="w-full mb-4 h-12" onClick={() => signIn("google", {
                callbackUrl:
                    "/dashboard"
            })}>
  ...
  3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
   ...
   0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55     
  1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1       
  ...
  text-white">
            ...
            <p className="text-center text-sm mt-6">
                Don't have an account? <Link href="/signup" className="text-primary hover:underline font-medium">Sign
                    up</Link>
            </p>
        </div>
    )
